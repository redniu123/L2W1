...
def run_l2w1_inference(
    ...
    try:
        ...
        if line_results:
            ...
            # Get entropy
            avg_entropy = sum(r.get("avg_entropy", 0) for r in line_results) / len(line_results)
...
def print_results(results: Dict[str, Any]) -> None:
    ...
    print(f"  | CER | {cer_a*100:.2f}% | {cer_f*100:.2f}% | {cer_sign} {abs(cer_delta)*100:.2f}% |")
    print(f"  | Accuracy | {acc_a*100:.2f}% | {acc_f*100:.2f}% | {acc_sign} {abs(acc_delta)*100:.2f}% |")
    print()
